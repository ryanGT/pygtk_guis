...
class RTP_motor_gui(object):
    ...
    def swept_sine_test(self, widget, data=None):
        ...
        min_freq_text = self.ss_min_freq_entry.get_text()
        min_freq = float(min_freq_text)
        max_freq_text = self.ss_max_freq_entry.get_text()
        max_freq = float(max_freq_text)

        amp_text = self.ss_amp_entry.get_text()
        amp = int(amp_text)

        dur_text = self.ss_dur_entry.get_text()
        duration = int(dur_text)
        
        kwargs = {'amp':amp, 'minf':min_freq, 'maxf':max_freq, 'plot':False,\
                  'stopn':duration, 'plot':False}
                  
        #self.test.Swept_Sine(**kwargs)
        #self.test.Notched_Swept_Sine(G_notch, **kwargs)
        self.test.Close_Serial()
        self.test.Swept_Sine(**kwargs)
        self.plot_results(test=self.test, legloc=3)
...
##                   legloc=5)
    ...
    def __init__(self):
        ...
        self.save_button = gtk.Button("Save Data")        
        ...
        ol_hbox.pack_end(self.ol_amp_entry, *pack_args_pad)
        ol_hbox.pack_end(amp_label, *pack_args_pad)
        #ol_hbox.pack_start(amp_label, False)
        #ol_hbox.pack_start(self.ol_amp_entry, False)
        ol_dur_box = gtk.HBox(homogeneous=False)
        ol_dur_box.pack_end(self.dur_entry, *pack_args_pad)
        ol_dur_box.pack_end(dur_label, *pack_args_pad)
        ...
        fsvbox.show()
        fsvbox.pack_start(fs_label, *args_no_pad)
        fshbox1 = gtk.HBox(homogeneous=False)
        fshbox1.pack_end(self.fs_amp_entry, *pack_args_pad)
        fshbox1.pack_end(fs_amp_label, *pack_args_pad)
        fsvbox.pack_start(fshbox1, *args_no_pad)
        fshbox2 = gtk.HBox(homogeneous=False)
        fshbox2.pack_end(self.fs_freq_entry, *pack_args_pad)
        fshbox2.pack_end(fs_freq_label, *pack_args_pad)
        fsvbox.pack_start(fshbox2, *args_no_pad)
        fshbox3 = gtk.HBox(homogeneous=False)
        fshbox3.pack_end(self.fs_dur_entry, *pack_args_pad)
        fshbox3.pack_end(fs_dur_label, *pack_args_pad)
        fsvbox.pack_start(fshbox3, *args_no_pad)
        self.fixed_sine_button = gtk.Button("Fixed Sine")
        fsvbox.pack_start(self.fixed_sine_button, False, False, 10)
        ...
        self.save_button.connect("clicked", self.save_data, None)
        ...
        # Swept Sine
        swept_sine_vbox = gtk.VBox(homogeneous=False, spacing=5)
        self.ss_amp_entry = gtk.Entry()
        self.ss_amp_entry.set_max_length(7)
        self.ss_amp_entry.set_size_request(Entry_width, -1)
        self.ss_amp_entry.set_text("100")
        self.ss_min_freq_entry = gtk.Entry()
        self.ss_min_freq_entry.set_max_length(7)
        self.ss_min_freq_entry.set_size_request(Entry_width, -1)
        self.ss_min_freq_entry.set_text("0")
        self.ss_max_freq_entry = gtk.Entry()
        self.ss_max_freq_entry.set_max_length(7)
        self.ss_max_freq_entry.set_size_request(Entry_width, -1)
        self.ss_max_freq_entry.set_text("15")
        self.ss_dur_entry = gtk.Entry()        
        self.ss_dur_entry.set_max_length(10)
        self.ss_dur_entry.set_size_request(Entry_width+10, -1)
        self.ss_dur_entry.set_text("1000")
        ss_label = gtk.Label('Fixed Sine')
        ss_amp_label = gtk.Label("amp (counts):")
        ss_min_freq_label = gtk.Label("min. freq. (Hz):")
        ss_max_freq_label = gtk.Label("max. freq. (Hz):")
        ss_dur_label = gtk.Label("duration:")
        swept_sine_vbox.pack_start(ss_label, *args_no_pad)
        sshbox1 = gtk.HBox(homogeneous=False)
        sshbox1.pack_end(self.ss_amp_entry, *pack_args_pad)
        sshbox1.pack_end(ss_amp_label, *pack_args_pad)
        swept_sine_vbox.pack_start(sshbox1, *args_no_pad)
        sshbox2 = gtk.HBox(homogeneous=False)
        sshbox2.pack_end(self.ss_min_freq_entry, *pack_args_pad)
        sshbox2.pack_end(ss_min_freq_label, *pack_args_pad)
        swept_sine_vbox.pack_start(sshbox2, *args_no_pad)
        sshbox3 = gtk.HBox(homogeneous=False)
        sshbox3.pack_end(self.ss_max_freq_entry, *pack_args_pad)
        sshbox3.pack_end(ss_max_freq_label, *pack_args_pad)
        swept_sine_vbox.pack_start(sshbox3, *args_no_pad)
        sshbox4 = gtk.HBox(homogeneous=False)
        sshbox4.pack_end(self.ss_dur_entry, *pack_args_pad)
        sshbox4.pack_end(ss_dur_label, *pack_args_pad)
        swept_sine_vbox.pack_start(sshbox4, *args_no_pad)
        
        swept_sine_vbox.pack_start(self.swept_sine_button
                                   , False, False, 0)        
        ss_label = gtk.Label("Swept")
        self.notebook.append_page(swept_sine_vbox, ss_label)
       
        #button_vbox.pack_start(self.swept_sine_button, False, False, 0)
        
        button_vbox.pack_start(self.notebook, False)
        button_vbox.pack_start(sep1, False)
        step_vbox = gtk.VBox(homogeneous=False, spacing=5)
        #button_vbox.pack_start(self.vib_check, False)
        step_vbox.pack_start(vib_label, False)
        step_vbox.pack_start(self.vib_on_radio, False)
        step_vbox.pack_start(self.vib_off_radio, False)
        step_vbox.pack_start(sep2, False)
        step_vbox.pack_start(self.step_button, False, False, 0)
        ...
        fixed_sine_label = gtk.Label("Fixed")
        self.notebook.append_page(fsvbox, fixed_sine_label)
        #Fixed Sine Stuff
        #button_vbox.pack_start(fsvbox, False)

        step_label = gtk.Label("Step")
        self.notebook.append_page(step_vbox, step_label)

        ol_step_vbox = gtk.VBox(homogeneous=False, spacing=5)
        #button_vbox.pack_start(sep3, False)
        #button_vbox.pack_start(sep4, False)
        ol_step_vbox.pack_start(ol_label, False)
        ol_step_vbox.pack_start(ol_hbox, False)
        ol_step_vbox.pack_start(ol_dur_box, False)
        ol_step_vbox.pack_start(self.ol_step_button, False)
        ol_step_label = gtk.Label("OL Step")
        self.notebook.append_page(ol_step_vbox, ol_step_label)
        ...
        button_vbox.pack_start(self.save_button, False)
        ...
        self.window.set_title('RTP Motor GUI v. 1.1')
        # and the window
        #self.window.show()
